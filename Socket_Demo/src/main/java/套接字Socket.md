...
public class SocketDemo1 {
    public static void main(String[] args) {
        try {
            ...
            System.out.println("项目根目录：" + PATH);//一会从项目根目录里拿一张图片给浏览器看
            //初始化服务端socket并且绑定9999端口
            ServerSocket serverSocket = new ServerSocket(9999);
            //serverSocket.setSoTimeout(10000);
            //等待客户端的连接
            Socket socket;
            while ((socket = serverSocket.accept()) != null) {//可以多个访问
                //获取输入流
                ...
                while (!(str = bufferedReader.readLine()).equals("")) {
                    if (str.contains("GET")) {						 //01234       987654321
                        URLPATH = str.substring(4, str.length() - 9);//GET /---.jpg HTTP/1.1
...
//        BufferedReader bufferedReader = new BufferedReader(inputStreamReader);
        ...
        while((len = inputStream.read(temp)) != -1) {
            System.out.println(new String(temp,0,len,"GBK"));//上古服务器，编码没讲究
        }
        //要关闭各种
